...
// Create a result class with field we want to set to the object
    ...
    $result->accesscode = $poll[$idPoll]->{'accesscode'};
    $result->anonymous = $poll[$idPoll]->{'anonymous'};
 ...
 * @return mixed
 */
function evoting_vote($userVoteId, $optionId, $idPoll, $lang, $userid = null) {
    global $DB;
...
function evoting_extend_settings_navigation(settings_navigation $settingsnav, navigation_node $workshopnode = null) {
    global $PAGE, $COURSE;
    $context = context_system::instance();
    if (has_capability('mod/evoting:addinstance', $context)) {
        $coursenode = $PAGE->navigation->find($COURSE->id, navigation_node::TYPE_COURSE);
        $childnode = $coursenode->create(get_string('pluginname', 'mod_evoting'), new moodle_url('/mod/evoting/reports.php'));
        $coursenode->add_node($childnode, 'grades');
    }
}
/**
...
/*
 * Get evoting course modules
 */

function mod_evoting_course_module($courseid) {
    global $DB;
    $modulename = "evoting";
    $params['courseid'] = $courseid;
    $params['modulename'] = $modulename;
    $sql = "SELECT cm.*, m.name, md.name AS modname, m.accesscode 
              FROM {course_modules} cm
                   JOIN {modules} md ON md.id = cm.module
                   JOIN {" . $modulename . "} m ON m.id = cm.instance                   
             WHERE md.name = :modulename 
                   AND cm.course = :courseid AND anonymous = 0";

    return $DB->get_records_sql($sql, $params);
}

function mod_evoting_is_anonymous($evoting_id) {
    global $DB;
    $is_anonymous = $DB->get_record('evoting', array('id' => $evoting_id), 'anonymous');
    if ($is_anonymous->anonymous == 1) {
        return true;
    } else {
        return false;
    }
...
function mod_evoting_permod_questions($evoting_id) {
    global $DB;
    $questions = $DB->get_records('evoting_questions', array('evotingid' => $evoting_id));
    $count = 0;
    if ($questions) {
...
function correct_answer_by_user($evotingid, $userid) {
    global $DB;
    $correctanswers_by_user_count = 0;
    if (!mod_evoting_is_anonymous($evotingid)) {
        $useranswers = $DB->get_records('evoting_answers', array('evotingid' => $evotingid, 'userid' => $userid));
        $sql = "SELECT *  FROM {evoting_questions}  AS `evques`"
                . " INNER JOIN {evoting_options} AS `ev_opt` ON "
                . "evques.`id` = ev_opt.`evotingquestionid`"
                . " WHERE ev_opt.correct = 1"
                . " AND evques.`evotingid` = " . $evotingid;
        $correctanswers = $DB->get_records_sql($sql, null);
        foreach ($useranswers as $answers) {
            if (array_key_exists($answers->optionid, $correctanswers)) {
                $correctanswers_by_user_count++;
            }
        }
    }
    $answer_percent = 0;
    if ($correctanswers_by_user_count) {
        $answer_percent = ($correctanswers_by_user_count / mod_evoting_permod_questions($evotingid) * 100);
    }
...
function is_student() {
    ...
    $isstudentanywhere = $DB->record_exists('role_assignments', ['userid' => $USER->id, 'roleid' => $roleid]);
    if (isloggedin() && $isstudentanywhere) {
        return true;
    } else {
        return false;
    }
