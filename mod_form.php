...
class mod_evoting_mod_form extends moodleform_mod {
	...
	public function definition() {
		...
		$mform -> setType('idCourse', PARAM_TEXT);
                ...
                $mform->addElement('hidden', 'edit', optional_param('update', 0, PARAM_INT));
		...
		if (!empty($CFG -> formatstringstriptags)) {
			...
			$mform -> setType('name', PARAM_CLEAN);
                ...
                $mform->addElement('advcheckbox', 'anonymous', 'Anonymous E-Voting'); //Record vote Anonymous or not
		// Poll name required
		...
		$mform -> addRule('name', get_string('maximumchars', '', 255), 'maxlength', 255, 'client');
                ...
                $availablefromgroup[] =& $mform->createElement('text', 'accesscode', '');
                $availablefromgroup[] =& $mform->createElement('checkbox', 'availablefromenabled', '', get_string('enable'));
                $mform->addGroup($availablefromgroup, 'accesscode', 'Access Code', ' ', false);
                $mform->disabledIf('accesscode', 'availablefromenabled');
                $mform -> setType('accesscode', PARAM_RAW);
                $data = '123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcefghijklmnopqrstuvwxyz';
                $defultaccesscode = substr(str_shuffle($data), 0, 6);
                if(isset($this->current->accesscode)){ 
                   $mform->setDefault("accesscode", $this->current->accesscode);
                }else{
                     $mform->setDefault("accesscode", $defultaccesscode);
                }
//                  $mform -> addRule('accesscode', null, 'required', null, 'server');
//		$mform -> addRule('accesscode', get_string('maximumchars', '', 100), 'maxlength', 100, 'server');
   ...
   //Custom validation should be added here
    function validation($data, $files) {
        ...
        if(empty($data['accesscode'])){
            $errors['accesscode'] = "Please input access code.";
            
        }else if($data['edit'] == 0 && $DB->record_exists('evoting', array('accesscode'=> $data['accesscode']))){
              $errors['accesscode'] = "This access code taken.please enter new";
        }
