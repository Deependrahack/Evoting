...
//} else {
  //   define('QRCODE_URL_LINK', "check.php?id=" . $id);
//}
...
if(!has_capability('mod/evoting:openevoting', $context_course)){
    if(!empty($evoting->accesscode)){
        $rediurl =new moodle_url('/mod/evoting/check.php', array('id'=> $id));
        redirect($rediurl);      
     die;
    }
	...
	$checkurl =new moodle_url('/mod/evoting/check.php', array('id'=> $id));
        echo !empty($evoting->accesscode) ? '<h3>Access code: '.html_writer::span($evoting->accesscode).'</h3> ('.$checkurl.')' : '';
